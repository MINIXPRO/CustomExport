...
# ---------------------------------------------------------------------------
# Template fields for Sales Invoice Sub Items (custom_sub_items child table).
...
@frappe.whitelist()
def get_si_sub_items_template(sales_invoice=None):
    """
    Generate and return an XLSX template for Sales Invoice Sub Items
    bulk import/export.

    Row 1 → column labels  (human-readable, styled dark-blue)
    Row 2 → fieldnames     (machine-readable, styled mid-blue)
    Row 3+ → existing sub item data when `sales_invoice` is provided.
    """
    try:
    ...
    if sales_invoice:
        try:
            ...
            for sub in (si.custom_sub_items or []):
                row = []
    ...
    fname = (
        f"si_sub_items_{sales_invoice}.xlsx"
        if sales_invoice
        else "si_sub_items_template.xlsx"
    )
...
@frappe.whitelist()
def import_si_sub_items(sales_invoice, file_url):
    """
    Import Sales Invoice Sub Item rows from the custom XLSX / CSV template.
    ...
    for parent_item, sub_item_code, row_data in parsed_rows:
        ...
        if idx < len(candidates):
            ...
            base.pop("name", None)
            base.pop("idx", None)
            new_sub = si.append("custom_sub_items", base)
        else:
            new_sub = si.append("custom_sub_items", {
    ...
    return {
        "total":   total,
        "message": f"Successfully imported {total} sub item row(s) into {sales_invoice}. Sub items replaced.",
    }
