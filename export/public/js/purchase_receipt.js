    ...
    validate(frm) {
        ...
        if (frm.doc.custom_sub_items) {
            frm.doc.custom_sub_items.forEach(row => {
                calculate_sub_item_cif_values(frm, 'Purchase Receipt Sub Item', row.name);
            });
        }
...
function carry_forward_sub_items(frm) {
    ...
    // Get Purchase Order reference from first item's purchase_order field
    let purchase_order_ref = null;
    if (frm.doc.items && frm.doc.items.length > 0) {
        purchase_order_ref = frm.doc.items[0].purchase_order;
    }
    ...
    if (purchase_order_ref) {
        frappe.call({
            ...
            callback: function(r) {
                if (r.message) {
                    // Carry forward order type from PO
...
frappe.ui.form.on("Purchase Receipt Sub Item", {
    rate(frm, cdt, cdn) {
...
/************************************
 * CALCULATE PR CIF TOTALS (HEADER)
 ************************************/
function calculate_pr_cif_totals(frm) {
